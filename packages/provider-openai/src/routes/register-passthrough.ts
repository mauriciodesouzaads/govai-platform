...
function allowedMethodsFor(pathTemplate: string): string[] {
  return ROUTE_METHODS.filter((m) => {
    const r = resolveOpenAICapabilityForRequest({ method: m, pathTemplate, isStream: false });
    if (r.capability_id === 'unknown') return false;
    const cap = OPENAI_CAPABILITIES.find((c) => c.id === r.capability_id);
    return (
      cap !== undefined &&
...
export async function registerOpenAIPassthrough(
  ...
  app.all<{ Params: { '*': string } }>(
    ...
    async (req, reply) => {
      ...
      // Tool classifier (only on /v1/responses or /v1/chat/completions with
      // body.tools[]) — runs BEFORE the beta floor so a blocked event always
      // carries the classifications.
      let toolClassifications: ReturnType<typeof classifyOpenAITools>['classifications'] = [];
      let toolBlock: Extract<ReturnType<typeof classifyOpenAITools>, { decision: 'block' }> | null =
        null;
      ...
      // default; deny ONLY the explicit hard_denied floor.
      const betaHeaderRaw =
