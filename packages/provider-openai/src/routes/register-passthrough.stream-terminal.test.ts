// EP-008C — OpenAI passthrough stream terminal-completeness e2e.
// Mirror of the Anthropic passthrough test: drives the REAL handler over a real
// socket with a controllable (mocked) upstream + a capturing emitAuditEvent, and
...
const tenant: TenantContext = { org_id: ORG, tier: 'enterprise', operational_mode: 'production' };
...
beforeAll(async () => {
  ...
  await app.register(async (instance) => registerOpenAIPassthrough(instance, deps));
...
describe('EP-008C OpenAI passthrough stream terminal-completeness', () => {
