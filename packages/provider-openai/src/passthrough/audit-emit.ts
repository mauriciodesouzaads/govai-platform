// Audit event emit helpers for OpenAI passthrough — Decisão 4 / HAE-002 enforced
// + HAE-003 (purpose_deprecated / sunset_at / migration_target coherence).
...
export type BuildPassthroughInvokedInput = {
  ...
  /** HAE-003 fields — all three required together (Rule 7) when purpose_deprecated=true. */
  purpose_deprecated?: boolean;
  purpose_deprecation_sunset_at?: string;
  purpose_deprecation_migration_target?: string;
};

/**
 * Build (and validate) a `passthrough.invoked` v3 event. Throws if the schema
 * does not pass — the emitter is the last line of defense and must never
 * produce malformed events into the audit chain.
 */
  ...
  const ev = {
    ...
    tools_taxonomy_version:
      (input.detected_tool_classifications?.length ?? 0) > 0
        ? KNOWN_OPENAI_TAXONOMY_VERSION
        : undefined,
    purpose_deprecated: input.purpose_deprecated,
