...
import { betaObservationMarker, handleAnthropicBetaHeader } from './beta-header-handler.js';
...
describe('handleAnthropicBetaHeader — Native forward + observe (OD-1=A, M1)', () => {
  ...
  describe('BETA-08: marker safety', () => {
    it('an arbitrary client token is NEVER stored raw; the marker is bounded, hex-only and deterministic', async () => {
      ...
      const r1 = await handleAnthropicBetaHeader({
        org_id: randomUUID(),
        header_value: nasty,
        policy_table: policy([]),
        active_overrides_loader: noOverrides,
      });
      const r2 = await handleAnthropicBetaHeader({
        org_id: randomUUID(),
        header_value: nasty,
        policy_table: policy([]),
        active_overrides_loader: noOverrides,
    ...
    it('betaObservationMarker is SHA-256 over the exact normalized token, per state', () => {
      ...
      expect(betaObservationMarker('hard_denied', 'computer-use-2025-11-24')).toBe(
