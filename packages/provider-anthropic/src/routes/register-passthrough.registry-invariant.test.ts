...
vi.mock('../capabilities/index.js', async (importOriginal) => {
  ...
  return {
    ...actual,
    resolveAnthropicCapabilityForRequest: (input: { method: string; pathTemplate: string; isStream: boolean }) => {
      const r = actual.resolveAnthropicCapabilityForRequest(input);
      // Corrupt ONLY the mapped POST resolution: a non-'unknown' id that is not in the registry.
      return r.capability_id === 'unknown' ? r : { ...r, capability_id: 'anthropic.bogus.not_in_registry' };
    },
  };
});

import { registerAnthropicPassthrough, type AnthropicPassthroughDeps } from './register-passthrough.js';
...
describe('ROUTE-04 — genuine registry inconsistency stays a loud 500', () => {
  it('mapped method + capability id absent from the registry → 500 capability_registry_missing (NOT 404/405), after auth', async () => {
    ...
    const res = await app.inject({
      ...
      payload: '{"model":"claude-x","max_tokens":1,"messages":[]}',
    });
    ...
    const res2 = await app.inject({ method: 'GET', url: '/passthrough/anthropic/v1/messages' });
