...
// `accept-encoding`, a real Fastify instance with registerAnthropicPassthrough
...
const PLAIN_JSON = Buffer.from(
  JSON.stringify({ id: 'msg_fixture', ok: true, pad: 'x'.repeat(2048), extra_provider_field: 'kept' }),
  'utf8',
);
const SSE_PLAIN = Buffer.from(
  [
    'event: message_start\ndata: {"type":"message_start","pad":"' + 'y'.repeat(1024) + '"}\n\n',
...
const USEFUL_HEADERS = {
  'anthropic-request-id': 'req_fake_enc_1',
...
const tenant: TenantContext = {
  org_id: '00000000-0000-4000-8000-0000000000e1',
...
describe('Anthropic passthrough — Content-Encoding transparency over real TCP (FB-3 / CT-005)', () => {
  ...
  it('ENC-01/05/06/07/08/09: gzip non-stream → decoded body, NO stale content-encoding/length, useful headers + request id survive, response hash over delivered bytes', async () => {
    ...
    expect(JSON.parse(res.body.toString('utf8'))).toMatchObject({ id: 'msg_fixture', ok: true });
    // ENC-05/06: useful headers + provider request id preserved
    expect(res.headers['content-type']).toBe('application/json');
    expect(res.headers['anthropic-request-id']).toBe('req_fake_enc_1');
    expect(res.headers['anthropic-ratelimit-requests-remaining']).toBe('41');
  ...
  it('ENC-01 (downstream real client): a Fetch client consuming GovAI\'s response gets valid JSON (would throw on a stale gzip header)', async () => {
    ...
    const res = await fetch(`http://127.0.0.1:${govPort}/passthrough/anthropic/v1/messages`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: MESSAGES_BODY,
    });
    ...
    expect(json['id']).toBe('msg_fixture');
  });
    ...
    for (const [coding, encoded] of [
      ...
      fakeResponse = {
        ...
        headers: {
          ...
          'anthropic-request-id': `req_${coding}`,
        },
        body: encoded,
      };
      const res = await rawRequest('/passthrough/anthropic/v1/messages', {
      ...
      expect(res.headers['content-encoding']).toBeUndefined();
      expect(res.body.equals(PLAIN_JSON)).toBe(true);
      expect(res.headers['anthropic-request-id']).toBe(`req_${coding}`);
  ...
  it('ENC-01 provider 4xx: a compressed provider ERROR body is relayed decoded with its status and truthful headers', async () => {
    const errJson = Buffer.from('{"type":"error","error":{"type":"invalid_request_error","message":"bad"}}', 'utf8');
