// EP-008C — Anthropic governed stream terminal-completeness e2e.
// Drives the REAL governed handler over a real socket with a controllable (mocked)
// upstream + a capturing emitAuditEvent (the emit lives inside the handler's
// finalize(outcome)). Proves the terminal PassthroughInvoked is emitted on EVERY
// termination path with the right stream_outcome + identity, once, never crashing.
...
async function postStream(signal?: AbortSignal): Promise<Response> {
  return fetch(`${govUrl}/governed/anthropic/v1/messages`, {
    ...
    body: '{"stream":true,"model":"claude-x","max_tokens":10,"messages":[{"role":"user","content":"hi"}]}',
    ...(signal ? { signal } : {}),
  });
}

describe('EP-008C Anthropic governed stream terminal-completeness', () => {
