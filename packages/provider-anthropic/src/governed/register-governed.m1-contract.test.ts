// Governed Anthropic surface — Foundation V1 M1 contract:
...
import { registerAnthropicGoverned, type AnthropicGovernedDeps } from './register-governed.js';
import type { GovernedTenant } from './handle-messages.js';
import { SHA256_EMPTY } from '../passthrough/evidence-constants.js';

const sha = (b: Buffer) => createHash('sha256').update(b).digest('hex');
const OK = Buffer.from('{"id":"msg_1","content":[{"type":"text","text":"ok"}],"provider_field":"kept"}', 'utf8');
...
const MSG = (extra: Record<string, unknown> = {}) =>
  Buffer.from(JSON.stringify({ model: 'claude-x', max_tokens: 16, messages: [{ role: 'user', content: 'hi' }], ...extra }), 'utf8');
...
beforeAll(async () => {
  fake = http.createServer((req, res) => {
    ...
    req.on('end', () => {
      ...
      if (fakeMode === 'sse') {
        res.writeHead(200, { 'content-type': 'text/event-stream', 'anthropic-request-id': 'req_s' });
        res.end(SSE);
      } else {
        res.writeHead(200, { 'content-type': 'application/json', 'anthropic-request-id': 'req_j' });
        res.end(OK);
      }
    });
  });
  ...
  const deps: AnthropicGovernedDeps = {
    ...
    resolveTenant: async (req) => ({
      org_id: '00000000-0000-4000-8000-0000000000d1',
...
describe('F2 — recommendation vs applied (additive HTTP honesty)', () => {
  it('F2-01: a forwarded governed request keeps x-govai-enforcement-decision as the matrix RECOMMENDATION and adds x-govai-enforcement-applied=forwarded; provider body unchanged', async () => {
    ...
    const body = MSG({ tools: [{ type: 'bash_20250124', name: 'bash' }] });
  ...
  it('F2-03: a computer-use tool block → 403 with block_trigger=tool_validation; the recommendation header stays the matrix label (not necessarily blocked)', async () => {
    const body = MSG({ tools: [{ type: 'computer_20250124', name: 'computer' }] });
    ...
    expect(j['reason']).toBe('tool_blocked:anthropic_provider_hosted_computer_use:capability_blocked_via_token');
...
describe('FB-2 governed — non-computer tools reach governance (no stale pre-block)', () => {
  it('code_execution + typed_unknown + custom on enterprise/production → forwarded; matrix recommendation from real risk (enforce for C)', async () => {
    const body = MSG({
      tools: [
...
describe('H-2 — governed stream detection is top-level only', () => {
  it('a NESTED "stream": true inside message content is NOT a streaming request (non-stream forward, is_stream=false)', async () => {
    const body = MSG({ messages: [{ role: 'user', content: 'please set "stream": true in your reply' }] });
    const res = await post(body);
    expect(res.status).toBe(200);
    expect(res.headers.get('content-type')).toBe('application/json');
    expect(Buffer.from(await res.arrayBuffer()).equals(OK)).toBe(true);
    expect(invoked()['is_stream']).toBe(false);
    expect(invoked()['capability_id']).toBe('anthropic.messages.create');
  });
  ...
  it('governed holds the ORIGINAL bytes: a whitespace/number-formatted body is forwarded byte-for-byte and native_request_hash attests the client bytes (no re-serialization)', async () => {
    const raw = Buffer.from(
      '{\n  "model" :  "claude-x",\n  "max_tokens": 16.0,\n  "temperature": 1.50,\n  "messages": [ { "role":"user", "content":"h\\u0069" } ],\n  "z_future": {"nested": [1,  2]}\n}',
      'utf8',
    );
  ...
  it('malformed JSON is NOT a new GovAI rejection — it is forwarded and the provider decides', async () => {
    const raw = Buffer.from('{"model":"claude-x", not json', 'utf8');
    const res = await post(raw);
    expect(res.status).toBe(200);
    expect(providerCalls).toBe(1);
    expect(capturedBody?.equals(raw)).toBe(true);
