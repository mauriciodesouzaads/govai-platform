...
- test files run by the `@govai/ui` vitest config (excluded from the root config because it is `environment: 'node'`): **31** files / **705** tests under `apps/ui/` (18 `*.test.ts` + 13 `*.test.tsx`; EP-B2 added `tests/identity.test.tsx`, U1.5 added the SSE / provider-adapter / conversation / receipt / transport suites under `src/features/ai/**` and `src/lib/api/client.stream.test.ts`, plus `tests/ai/{console,strictmode,receipt,markdown,persistence}.test.tsx`), executed by the CI `ui` job via `pnpm --filter @govai/ui test`
...
|---|---|---|
| `apps/ui` — static React+TS+Vite SPA, no BFF, no SSR | IMPLEMENTED_RUNTIME_SOURCE_AND_TEST_VERIFIED (EP-UIUX-V1-U1) | consumes the Fastify API directly on the same origin (`base: '/app/'`); `apps/ui/README.md`; 31 files / 705 tests via `pnpm --filter @govai/ui test`; CI `ui` job (typecheck, lint, test, build, bundle secret scan) |
