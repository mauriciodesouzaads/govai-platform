...
- test files run by the `@govai/ui` vitest config (excluded from the root config because it is `environment: 'node'`): **15** files / **324** tests under `apps/ui/` (7 `*.test.ts` + 8 `*.test.tsx`; EP-B2 added `tests/identity.test.tsx`), executed by the CI `ui` job via `pnpm --filter @govai/ui test`
...
|---|---|---|
| `apps/ui` — static React+TS+Vite SPA, no BFF, no SSR | IMPLEMENTED_RUNTIME_SOURCE_AND_TEST_VERIFIED (EP-UIUX-V1-U1) | consumes the Fastify API directly on the same origin (`base: '/app/'`); `apps/ui/README.md`; 15 files / 324 tests via `pnpm --filter @govai/ui test`; CI `ui` job (typecheck, lint, test, build, bundle secret scan) |
