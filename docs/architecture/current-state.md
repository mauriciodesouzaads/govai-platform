...
| DB migrations | `apps/api/src/db/migrations/*.sql` | 35 |
...
- **Canonical P0-C carry-forward register** (each entry keeps its sealed-record classification;
  ...
  **★ UPDATED BY P0-D1 (this tree):** two rows above are re-dispositioned in the P0-D1
  canonical section at the end of this document — the historical wording above is preserved,
  not rewritten:
  ```
  R1_DURABLE_CONTEXT_P1            = CLOSED FOR anthropic_messages + openai_responses
                                     (server-assembled durable context is the executor's
                                     dispatch path; the pipelining gate is proven at the wire)
                                     / OPEN FOR codex + claude_code (P0-D2)
  BOUNDARY_CAUSAL_VERSION_SNAPSHOT = RE-EXAMINED AND CLOSED for the two D1 surfaces: the
                                     boundary CAS's causal_version predicate now certifies
                                     the freshness of SERVER-ASSEMBLED context (version-first
                                     sampling before the projection read; a stale build loses
                                     the CAS pre-POST and rebuilds)
  ```
...
- **NEXT: P0-D provider continuation.** The intervening program-level architectural movement
  ...
  **★ EXECUTED AND MERGED — see the movement's canonical section below.** Its
  ...
  `f998f55aee405adbc12c762f237854f118b8939c`). **★ P0-D has since STARTED: its first
  submovement P0-D1 is implemented in this tree — see the P0-D1 canonical section below.**
...
- **`NATIVE_EXPERIENCE_CONTRACT_AND_CURRENT_BASELINE=COMPLETE_AND_MERGED` — documentary/
  ...
  documentation validation. The movement was independently reviewed and owner-authorized,
  and PR #149 was squash-merged as `f998f55aee405adbc12c762f237854f118b8939c` (tree
  `86093763952a4fb5dbb38ee6c7ef736f21f9c870` — byte-identical to the final independently
  reviewed tree; post-merge main CI run `33288386315` SUCCESS). The Native Experience
  Contract V1 is the CURRENT normative contract; parity V2 is the CURRENT research
  baseline; V1 remains the byte-preserved historical snapshot.
...
- **Explicit non-claims:** no runtime capability became implemented because its contract
  now exists; at that movement's anchor `P0_D_PROVIDER_CONTINUATION=NOT_STARTED`,
...
- **NEXT IMPLEMENTATION MOVEMENT: `P0-D — PROVIDER CONTINUATION`** — this movement's
  ...
  inert admission are now in this tree too, merged as PR #153 (its §18.3 obligation is DISCHARGED,
  with the legacy qualification recorded there), while P0-D2's coding-harness RUNTIME remains
  NOT_STARTED and is owned by P5/P6 — see the P0-D2 canonical section below.**

### EP-AI-CONVERSATION-CONTINUITY-V1-01 — P0-D1 canonical state (this tree)

- **`P0_D_PROVIDER_CONTINUATION=IN_PROGRESS` / `P0_D1_DURABLE_CONTEXT_API_PROVIDER_CONTINUATION=COMPLETE` / `P0_D2_ARCHITECTURE_AND_INERT_ADMISSION=MERGED` / `P0_D2_CODING_HARNESS_RUNTIME=NOT_STARTED`**
  (EP-AI-CONVERSATION-CONTINUITY-V1-P0-D1-DURABLE-CONTEXT-API-PROVIDER-CONTINUATION-01).
  Base: main `bf78bb0bbc8298466831664958cd91514d42261a` (tree `1ab1ed10…`), technical PR **#151**,
  final frozen head `63221f0d3290fc02d3a99f15e3b14592f596c92f` (tree
  `ff99c89e3e427c95a4b2634fb3859f088ecf5891`), squash-merged to main as
  `700aad9631f91d0655bd98aa113fbc59d74f88b0` — whose tree IS that reviewed tree, byte for byte,
  so what landed is exactly what was reviewed. `COMPLETE` is written here only because the
  exact-head review closed clean and the frozen tree merged identically; it is a record of that
  review and of the external post-merge proof, not a claim made on the executor's own authority.
  **`P0_D` itself is NOT complete** — P0-D2 is the remaining half. Provider facts
  reverified against CURRENT first-party documentation 2026-08-30 with NO material drift from
  the 2026-08-21/2026-08-29 snapshots (Anthropic `/v1/messages` stateless full-history replay;
  thinking blocks passed back unmodified, per-model keep-all vs auto-strip is PROVIDER
  behavior, strip on model switch; OpenAI `previous_response_id` ⊕ `conversation` mutually
  exclusive; `store` default true / ~30-day retention; encrypted reasoning items emitted by
  default in stateless mode and replayable verbatim).
- **Completion evidence — every anchor, immutably.**
  ```
  TECHNICAL_PR             = #151  (feat(ai-conversations): add durable context for API
                             provider continuation)
  FROZEN_REVIEWED_HEAD     = 63221f0d3290fc02d3a99f15e3b14592f596c92f
  FROZEN_REVIEWED_TREE     = ff99c89e3e427c95a4b2634fb3859f088ecf5891
  EXACT_HEAD_CI            = 33546836184  (pull_request; unit / ui / integration all success)
  CODEX_EXACT_HEAD         = CLEAN on 63221f0d — delivered as an ISSUE COMMENT by the genuine
                             chatgpt-codex-connector bot (a clean pass is a comment, not a
                             review object); 42 review threads, 0 current-and-unresolved
  SQUASH_MERGE_SHA         = 700aad9631f91d0655bd98aa113fbc59d74f88b0
  MERGE_PARENT             = bf78bb0bbc8298466831664958cd91514d42261a
  SQUASH_MERGE_TREE        = ff99c89e3e427c95a4b2634fb3859f088ecf5891
  TREE_IDENTITY            = PASS
  POST_MERGE_MAIN_CI_RUN   = 33580661031
  POST_MERGE_MAIN_CI       = SUCCESS  (push event on the merge SHA; unit / ui / integration.
                             The A2 forbidden-strings step is PR-lane only — `ci.yml` gates it
                             on `github.event_name == 'pull_request'` — so its SKIP on a push
                             event is by design, not a failure)
  ```
  The sealed evidence records live outside the repository under
  `/Users/Shared/govai-handoff/audits/ai-conversation-continuity-v1/p0-d1-provider-continuation/`
  (the RF-3 / RF-4 / RF-5 closure handoffs + SHA-256 sidecars, the independent provider-validator
  audit, the protocol-grammar closure sweep, and the frozen-tree merge-report recovery
  `PR151-P0-D1-FROZEN-TREE-MERGE-REPORT-RECOVERY-01.md`, SHA-256
  `cd97a8757e4176d1533d5bb88670263c904678520a5de67663a6a3badafb3866`, which recovers a record
  lost to host sleep during the merge session's final report assembly). They are byte-frozen;
  corrections to them are recorded as ledger entries in
  [stale-docs-register.md](./stale-docs-register.md), never as edits.
- **What is implemented (runtime, worker plane only — the request plane is unchanged):**
  - **Server-assembled durable branch context** (`execution/durable-context.ts`): a
    deterministic two-phase builder — Phase A loads encrypted rows inside the step-3 load
    transaction (fork-ancestry walk applying §3 boundary modes incl. the §7.5 fork-pin
    exemption; per-turn rule: user items ALWAYS, output ONLY from the current, completed,
    non-excluded attempt); Phase B decrypts holding NO database client. Version-first §7.8
    sampling + the boundary CAS's `causal_version` predicate make stale context structurally
    unPOSTable.
  - **The §11 `ProviderConversationAdapter` boundary** (`execution/adapters/`): pure,
    per-provider strategies behind a responsibilities interface — no normalized conversation
    protocol, no model input, no silent substitution (LAW NX-1/NX-2/NX-5).
  - **Anthropic Messages adapter**: provider-native stateless replay; durable SSE reassembly
    (text/tool_use/thinking + signature/citations deltas; unknown event/delta types REFUSE);
    model-switch thinking strip exactly per first-party rule; `messages` is the ONLY replaced
    field — every other native control passes through verbatim (§30).
  - **OpenAI Responses adapter**: `previous_response_id` chaining when anchor exists +
    credential provenance matches + neither the anchor turn nor this turn set `store:false`;
    anchor derived from the durable projection (LAW 4 retry rewind and fork-tree chaining fall
    out structurally); stateless replay of full native item arrays (reasoning items verbatim)
    as the mandatory fallback; client-supplied `previous_response_id`/`conversation` in a
    stored config is an explicit `rejected` refusal (`continuation_conflict`), never honored
    and never silently stripped.
  - **Encrypted continuation-anchor persistence**: the exact parent anchor an attempt chained
    FROM is envelope-encrypted (purpose `conversation_content`) and stamped in the
    dispatch-boundary commit — the one write site 0031's post-boundary causal freeze admits;
    a restore→rebuild lawfully re-stamps (including back to NULL).
  - **Migration `0036_ai_conversation_provider_continuation.sql`**: exactly two column
    authorities (worker SELECT on branch fork columns; worker UPDATE — write-only — on the
    four `continuation_parent_*` columns). No new table/column/index/policy/trigger/function;
    NO new `ai_conversation_provider_state` privilege for any role (migration `0031`'s existing
    inert `govai_app` SELECT + INSERT on that table is unchanged — F-01 in the P0-D2 section).
- **Dispositions (each source-adjudicated in the movement, none cosmetic):**
  ```
  R1_DURABLE_CONTEXT_P1_API_SURFACES   = CLOSED (anthropic_messages + openai_responses; the
                                         §35 pipelining gate is proven at the wire in
                                         tests/integration/ai-conversation-durable-context.test.ts)
  R1_DURABLE_CONTEXT_P1_CODING_HARNESS = OPEN / P0-D2 (codex + claude_code)
  BOUNDARY_CAUSAL_VERSION_SNAPSHOT     = RE-EXAMINED AND CLOSED for the two D1 surfaces
                                         (the predicate now certifies assembled-context freshness)
  OPENAI_PREVIOUS_RESPONSE_ID          = IMPLEMENTED (all §20/§21 conditions enforced)
  OPENAI_CONVERSATION_OBJECTS          = DEFERRED_WITHIN_P0D
                                         (EXPLICIT_POLICY_SIGNAL_EXISTS=NO — the accepted
                                         architecture gates provider-stored state on a tenant
                                         policy signal absent from executable source; nothing
                                         invented)
  ...
  CONTEXT_AGGREGATE_BUDGET             = ENFORCED IN PHASE A (512 turns / 4096 items / 32 MiB
                                         ...
                                         explicit `rejected` (`context_budget_exceeded`) —
                                         REFUSAL, never silent truncation; compaction is a
                                         §11 adapter capability of a later movement
  MODEL_ID_AGNOSTICISM                 = PRESERVED (no model input anywhere in the adapter
                                         registry; regression-pinned)
  PROVIDER_EXACTLY_ONCE                = NOT_CLAIMED (permanent)
  ```
- **Explicit non-claims for P0-D1:** no Codex/Claude Code continuation (P0-D2); no public
  Retry endpoint (retry-minted attempts are honored by the context/anchor rules, proven by
  seeded-state tests); no public Stop terminalization arm; no §19 Delete protocol; no
  evidence-link materialization (P0-F); no persistent workspace UI (P0-E — the AI Console
  transcript remains memory-only); no OpenAI conversation objects; no provider-held shared
  continuation state of any kind (the §11 taint/rotation discipline is vacuously satisfied and
  arrives WITH the first strategy that creates such state). Every other P0-C carry-forward
  (`P3-1-CHAT-COMPLETIONS-STREAM-GATE-ASYMMETRY`, `DIRECT_HTTP_WRITABLE_BACKPRESSURE`,
  `WORKER_DEPLOYABLE_BUNDLE_DOCKER`, `PUBLIC_STOP_TERMINALIZATION_ARM`,
  `DELETE_ROOT_LOCK_DISCIPLINE`, `STREAM_OUTCOME_FENCED_EXIT_VOCABULARY`,
...
- **NEXT after P0-D1 WAS `P0-D2 — CODING HARNESS CONTINUATION`, and its ARCHITECTURE half is
  merged (PR #153)** — see the P0-D2 canonical section below. **★ The phrase "on this same
...
- **`P0_D_PROVIDER_CONTINUATION=IN_PROGRESS` / `P0_D1_DURABLE_CONTEXT_API_PROVIDER_CONTINUATION=COMPLETE` / `P0_D2_ARCHITECTURE_AND_INERT_ADMISSION=MERGED` / `P0_D2_CODING_HARNESS_RUNTIME=NOT_STARTED`**
  (EP-AI-CONVERSATION-CONTINUITY-V1-01 P0-D2). This section describes what is implemented **in
  this tree**. Base: main `0d511c8171c384716c366b5b52a6888ed3a2dd20` (tree `313118bd…`),
  technical PR **#153**, frozen reviewed head `475259c848b7b24d727376a4a7b0db41b116c4d6` (tree
  `b977c8a11b031fa0ded34debdff2324502e267cb`), squash-merged to main as
  `71689db8d703b53dbda46950f217349da11c6c54` — whose tree IS that reviewed tree, byte for byte,
  so what landed is exactly what was reviewed. `MERGED` is written here only on the strength of
  that external post-merge proof; it is not a completion claim. **`P0_D` itself is NOT
  complete** — the coding-harness runtime is not started, and the review's qualifications are
  recorded below exactly as issued.
- **Completion evidence — every anchor, immutably.**
  ```
  TECHNICAL_PR                      = #153  (feat(ai-conversations): define coding-harness
                                      continuation contracts and admission)
  FROZEN_REVIEWED_HEAD              = 475259c848b7b24d727376a4a7b0db41b116c4d6
  FROZEN_REVIEWED_TREE              = b977c8a11b031fa0ded34debdff2324502e267cb
  EXACT_HEAD_CI                     = 34698847707  (pull_request on 475259c8; unit / integration /
                                      ui all success)
  CODEX_REVIEW_STATE_AT_FROZEN_HEAD = review 5185064864 COMMENTED on commit
                                      83562b81034d7ff9d3518ff109261bb37f4687d6; its known P2
                                      ("call the canonical admission predicate") is remediated by
                                      the frozen head 475259c848b7b24d727376a4a7b0db41b116c4d6;
                                      thread PRRT_kwDOSTiHm86hs0Ww resolved;
                                      unresolved_non_outdated_threads = 0. NO exact-head Codex
                                      approval or review is claimed
  EXACT_HEAD_ASSURANCE_SOURCE       = the independent convergence review below, not the Codex
                                      review
  INDEPENDENT_REVIEW                = PASS_WITH_QUALIFICATIONS · BLOCKING_FINDINGS 0 ·
                                      HISTORICAL_EXECUTION_CONFORMANCE = QUALIFIED
  SQUASH_MERGE_SHA                  = 71689db8d703b53dbda46950f217349da11c6c54  (squash, merged
                                      2026-09-14T23:57:44Z, bound to expected head 475259c8)
  MERGE_PARENT                      = 0d511c8171c384716c366b5b52a6888ed3a2dd20  (tree 313118bd…)
  SQUASH_MERGE_TREE                 = b977c8a11b031fa0ded34debdff2324502e267cb
  TREE_IDENTITY / PARENT_IDENTITY   = PASS / PASS
  POST_MERGE_MAIN_CI_RUN            = 34911152172
  POST_MERGE_MAIN_CI                = SUCCESS  (push event on the merge SHA; unit / integration /
                                      ui; completed 2026-09-15T00:07:15Z)
  SOURCE_BRANCH                     = impl/p0d2-coding-harness-continuation-architecture,
                                      preserved at 475259c8
  OBS-01                            = RECORDED_PROCEDURAL_OBSERVATION — the merge executor's first
                                      G0 invocation aborted in its own evidence writer (a gate
                                      subdirectory did not yet exist) before any command capture,
                                      network call or evidence write; the writer was corrected
                                      locally and G0 was re-run in full and PASSED; no gate
                                      effect, no GitHub effect
  ```
  The sealed evidence records live outside the repository under
  `/Users/Shared/govai-handoff/audits/ai-conversation-continuity-v1/p0-d2-coding-harness-continuation/`:
  the independent convergence review
  `P0-D2-CANONICALIZATION-INDEPENDENT-CONVERGENCE-REVIEW-01-REPORT-01.md` (SHA-256
  `0a8bd14f69ad0a73de3b947b4e7524e8116436abc0726efaf9e62f8c9a9dfc9c`), the historical-qualification
  record `P0-D2-CODING-HARNESS-CONTINUATION-ARCHITECTURE-CANONICALIZATION-01-EXECUTION-REPORT-01-CORRECTION-02.md`
  (SHA-256 `84bc775a04175ce8f7bf292c7a0a5e7cfc1f057b5b0c87cd7c97d33e77369fa5`) and the frozen-tree
  merge execution report `P0-D2-FROZEN-TREE-MERGE-PR153-01-EXECUTION-REPORT-01.md` (SHA-256
  `05372b97d546bacfdfcf238d10ee0c02eecd6b96fd6969ed86bcb686b08d1d2d`). They are byte-frozen;
  corrections to them are recorded as ledger entries in
  [stale-docs-register.md](./stale-docs-register.md), never as edits.
- **Carried qualifications — recorded exactly as issued, none upgraded.**
  ```
  F-01 (P3) — ai_conversation_provider_state privilege wording; NOT CLOSED
    ...
    F-01                                 = CARRY_FORWARD_REQUIRED — future PROVIDER_STATE_PHYSICAL
                                           activation must revalidate and revoke/narrow, or
                                           formally prove non-bypass of, this privilege boundary
  F-02 (P3) — normative source anchors in native-experience-contract-v1.md §2
    F-02                                 = CARRY_FORWARD_REQUIRED AT ENTRY — PR #153 shifted four
                                           file:line anchors (the cited symbols were unchanged).
                                           In this tree those four anchors are rewritten to
                                           file + symbol form and no other line of that contract
                                           changed; this document does not declare F-02 closed
  F-03 (P3) — PR #153 body validation wording
    F-03                                 = SATISFIED by the squash commit message as sent; the
                                           PR #153 body was not edited
  HISTORICAL_EXECUTION_CONFORMANCE       = QUALIFIED (Q1–Q4, not upgraded; a green CI run and a
                                           successful merge are not retroactive proof)
  ```
...
- **Explicitly NOT implemented, and NOT claimed:** no coding-harness runtime, no native thread or
  ...
  `ai_conversation_provider_state` row and no privilege over that table added by P0-D2** (migration
  `0031`'s existing inert `govai_app` SELECT + INSERT on it is unchanged and is carried as F-01
  above), no worker/queue change, no new endpoint, no migration, no credential issuer, no
  supervisor, no delete/disposal execution, no UI.
