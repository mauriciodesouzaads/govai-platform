...
- **The `UI_UX_V1_FOUNDATION` product lane has STARTED, and its first milestone — U1, the evidence cockpit — is implemented in this tree** (`apps/ui`, EP-UIUX-V1-U1). It is a read-only static SPA over the three existing read surfaces (`/v1/evidence/*`, `/v1/audit-events`, `/v1/capabilities`); it changes NO backend behaviour (the Foundation V1 runtime anchor is unchanged) and it makes no capability claim the runtime does not support. See §1 *Interface layer*. **EP-UIUX-V1-B2 (this tree) adds the one backend surface U1 was missing: `GET /v1/me`** — a read-only projection of the identity `authenticateApiKey` already resolves — and wires it into the session, so the shell now shows the server-supplied operational mode, principal type and roles instead of correctly showing none. Production human auth/session/API-key lifecycle is still absent (residual R14) — `principal_type` is the literal `api_key` precisely so nothing presents a controlled-pilot org credential as a human login. **EP-UIUX-V1-U1.5 (this tree) adds the AI Console at `/ai`** — a provider-native conversational surface over the six already-registered direct provider routes (OpenAI Responses / Chat Completions and Anthropic Messages, each in Native/Audited and Governed mode), with **ZERO backend change**: no route, no migration, no event schema, no governance semantics. U2 (Workroom) is not started, and there is no governance-settings, Phase-5-enforcement, Workroom, regulatory or admin UI. **Two findings the AI Console's live acceptance produced are OPEN and belong to the backend, not to the UI** — `AI-CONSOLE-ORIGIN-RELAY-01` (the direct routes relay the browser's `Origin` header upstream; Anthropic rejects it with 401, which makes the Anthropic surface unusable from a browser) and `AI-CONSOLE-RESPONSES-DLP-GAP-01` (the governed Responses DLP pre-scan skips role-shaped `input[]` items). Both are described in [stale-docs-register.md](./stale-docs-register.md).
...
Counts from `find` **in this tree**: the M2A source commit (`de80664a`), plus the files added by the M3 documentary freeze (docs), plus the files added by EP-UIUX-V1-U1 (`apps/ui`) and EP-UIUX-V1-B2 (`routes/me.ts` and its two test files). M3 changed no code or test count; EP-B2 changes the route and test counts and nothing else:
...
- API route files (`apps/api/src/routes/*`): **19** (18 routes + `_not-implemented.ts`) — EP-B2 added `me.ts`
...
- test files run by the repository-root vitest config: **211** `*.test.ts` in this tree — **129** unit (under `apps/`+`packages/`; M1 added the content-encoding / native-contract / registry-invariant / transport-encoding / m1-contract suites, M2A added `request-id.test.ts`, `main-module.test.ts` and the two `query-fidelity` suites, EP-B2 added `apps/api/src/pipeline/auth.test.ts`), **77** under `tests/integration/` (EP-B2 added `me-route.test.ts`), **5** under `tests/live/` (live-gated, always excluded). U1.5 adds **no** root test file: it is UI-only. Since the PR #116 `GOVAI_INTEGRATION` config gate the default `pnpm test` is **unit-only** (129 files, **1463** tests, reproduced locally in this tree under Node 24); `pnpm test:integration` adds the integration files (CI runs both jobs; post-merge main CI run `31988375993` SUCCESS at `de80664a`)
- test files run by the `@govai/ui` vitest config (excluded from the root config because it is `environment: 'node'`): **31** files / **696** tests under `apps/ui/` (18 `*.test.ts` + 13 `*.test.tsx`; EP-B2 added `tests/identity.test.tsx`, U1.5 added the SSE / provider-adapter / conversation / receipt / transport suites under `src/features/ai/**` and `src/lib/api/client.stream.test.ts`, plus `tests/ai/{console,strictmode,receipt,markdown,persistence}.test.tsx`), executed by the CI `ui` job via `pnpm --filter @govai/ui test`
- operator-driven acceptance harness (NOT a vitest suite; excluded from both configs): `tests/acceptance/ai-console/**` — a real Postgres + real Fastify + real credential/KMS stack with either a provider-protocol loopback upstream (`pnpm acceptance:ai-console`) or the REAL providers (`pnpm acceptance:ai-console:live`)
...
|---|---|---|
| `apps/ui` — static React+TS+Vite SPA, no BFF, no SSR | IMPLEMENTED_RUNTIME_SOURCE_AND_TEST_VERIFIED (EP-UIUX-V1-U1) | consumes the Fastify API directly on the same origin (`base: '/app/'`); `apps/ui/README.md`; 31 files / 696 tests via `pnpm --filter @govai/ui test`; CI `ui` job (typecheck, lint, test, build, bundle secret scan) |
...
| U1.5 **open findings (backend, not UI)** | `OPEN` | **`AI-CONSOLE-ORIGIN-RELAY-01`** — the direct routes forward the browser's `Origin` header upstream (`origin` is in neither `HOP_BY_HOP` nor `STRIP_INBOUND_AUTH` in either provider package), and Anthropic answers `401 authentication_error: "CORS requests must set 'anthropic-dangerous-direct-browser-access' header"`. The Anthropic surface therefore **cannot work from a browser** until the relay is fixed server-side; no UI fix exists (a page cannot remove its own `Origin`) and the console must not assert that beta header, which would claim a direct browser access that is not happening. **`AI-CONSOLE-RESPONSES-DLP-GAP-01`** — `extractOpenAIResponsesText` descends only into `input[]` items carrying an explicit `type`, so role-shaped items are never DLP-scanned. Both measured against the running API; see [stale-docs-register.md](./stale-docs-register.md) |
