...
|---|---|---|---|
| Root unit | `pnpm test` (no `GOVAI_INTEGRATION`) | 148 | 1706 |
| Root integration-only | the identities `GOVAI_INTEGRATION=1` adds (proved set difference, all under `tests/integration/`) | 93 | 1452 |
| Root full integration gate | `pnpm test:integration` (unit + integration; the CI `integration` job) | 241 | 3158 |
