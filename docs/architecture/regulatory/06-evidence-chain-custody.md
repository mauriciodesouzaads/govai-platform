...
| Chain continuity across categories | BR-DP-01 Art. 50; BR-NET-05 | `ChainCategory` is fixed at `auth|run|policy|admin`; no new audit chain is created by later phases. | COVERED | `packages/core-events/src/index.ts` (`ChainCategory`, `chainIdFor`); Workroom and approval events on existing categories. | None at primitive level. |
