...
| Financial-data classification | BR-DP-01; BR-SEC-03 | Identify and protect financial data appropriately. | Baseline DLP detectors include `cpf` and `cnpj` (commonly present in financial flows) but do not constitute a financial-data classifier. | PARTIAL | `packages/dlp-br/src/baseline-detectors.ts`; `apps/api/src/pipeline/dlp.ts`. | Financial-data classifier with documented coverage; sector taxonomy. |
