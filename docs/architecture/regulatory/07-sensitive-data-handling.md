...
| Provider credentials / model keys (GovAI-specific) | BR-DP-01 Art. 46 | None at content level; out-of-band storage is dedicated. | None category-specific at content level. | Provider credentials at rest are envelope-encrypted with `dek_wrapped`; covered by `tests/integration/provider-credentials-plaintext-leak.test.ts`. | Operationally tenant-isolated; covered by RLS plus `0009_provider_credentials.sql` admin route guard. | COVERED for the storage primitive; PARTIAL for full lifecycle | Rotation cadence; per-tenant CMK / BYOK. |
