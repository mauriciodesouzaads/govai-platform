...
| Health-data classifier | BR-DP-01 | Detect and label health data. | PR-SD2A adds a conservative SIGNAL detector foundation in the `health_data` category: `cid10_code_candidate` (CID/ICD format with explicit context — **no** clinical-meaning mapping), `medical_record_identifier_candidate`, `prescription_context_candidate`, and `lab_result_context_candidate`. Findings are advisory rich metadata only: match-hash and redacted preview, no raw value, no enforcement coupling. SD2A is **not** a medical device, **not** a health-record system, **not** a telemedicine platform, and **not** clinical decision support. It does **not** infer diagnosis, triage, prognosis, treatment, or prescription correctness; it does **not** interpret lab values as normal, abnormal, high, low, or dangerous; it does **not** map CID/ICD codes to disease meaning; and it does **not** assert ANS / CFM / ANVISA / sector certification. Biometric, genetic, and other clinical sub-categories remain `GAP`. | PARTIAL | `packages/dlp-br/src/baseline-detectors.ts`; `packages/dlp-br/src/health-detectors.ts`; `apps/api/src/pipeline/dlp.ts`. | Clinical-data classifier with documented coverage and precision and recall posture; sector-specific policy ceiling; persisted classification. |
