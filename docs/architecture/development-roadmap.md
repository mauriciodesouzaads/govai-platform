...
Exit criteria: ADR-022..026 Accepted; idempotency decided; B3 plan reviewed.
...
Exit criteria: at least one governed OpenAI and one governed Anthropic direct runtime path have **test evidence** that emitted audit events are captured into the outbox; **or** an explicitly accepted decision documents why this wiring is deferred and which runtime path (`auditAppend` chain vs outbox) is authoritative.
...
Dependencies: Phase 2 (and Phase 2.5 decision). Note: **B3 seals captures already in the outbox; it is not the same as runtime-to-outbox dispatch, and its product-completeness depends on the Phase 2.5 wiring decision.**
