...
| `ADR-022`–`ADR-026` | (was) Status: Proposed | B3 decision-pack PR: ADR-022/024/025/026 → **Accepted** (design constraints, not impl authorization); **ADR-023 → Proposed/BLOCKED** (append→mark_sealed idempotency undecided) | HIGH | HIGH (B3) | ADR-023 must be resolved before any B3 code | yes — ADR-023 + explicit authorization remain B3 preconditions |
| Append→mark_sealed partial-failure idempotency (ADR-023) | open clause unresolved | **ADR-023 remains a blocker because append→mark_sealed partial failure is NOT solved by `capture_id` idempotency**; `auditAppend` has no per-capture key (`append.ts:72`). Three-layer distinction in technical plan §8.3 | HIGH (source-verified) | HIGH (B3) | Decide the level-3 mechanism (technical plan §8.3) before any B3 code | yes |
