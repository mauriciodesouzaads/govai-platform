...
- **D9** *(superseded 2026-08-08 — the note below replaces the WAS-state in both of its claims)*:
  ...
  - **Broken references, reconfirmed at `165291d9`:** 13 files cite ADR-016..019, including production source — `apps/api/src/db/migrations/0025_audit_capture_outbox_foundation.sql:36-37` (cites `docs/architecture/adr/ADR-017-audit-bridge-evidence-plane.md` and `docs/security/threat-model.md`), `packages/core-audit/src/capture.ts`, `packages/provider-anthropic/src/beta-policy.ts` — while the targets (`docs/architecture/adr/ADR-016..019`, `docs/security/threat-model.md`, `docs/architecture/claims-policy.md`) do not exist in the tree. Classification: `REPOSITORY_REFERENCE_TARGET_MISSING=YES`; `SOURCE_ARTIFACT_AVAILABLE_EXTERNALLY=YES`; `PROMOTION_PENDING=YES`. This is a documentary-integrity / repository-hygiene gap, **not** a P0.3-A runtime defect. Do **not** state "no broken document references exist" while the promotion is pending.
...
| (this register) D9 note | `D9_LOCATION=UNRESOLVED` + "no literal broken references reproduced (UNVERIFIED)" | superseded and falsified respectively — see the rewritten D9 note above (13 citing files incl. production source; targets missing; source available externally; promotion pending) |
