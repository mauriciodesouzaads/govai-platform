...
| `docs/architecture/adr/ADR-020-audit-sealer-runtime-model.md` | (was) role/session "Open design question"; Draft | ADR-022 resolves the role model | HIGH | MEDIUM (resolved) | **Done (B3 decision-pack PR):** ADR-020 → Superseded-in-part by ADR-022–026; B3 implementation not authorized | no longer the blocker; remaining blockers are Option A(b) implementation/tests, Phase 2.5 runtime-to-evidence dispatch, and explicit B3 authorization |
| `ADR-022`–`ADR-026` | (was) Status: Proposed | ADR-022/024/025/026 → **Accepted** design constraints; **ADR-023 → Accepted as design constraint (Option A(b))** — deterministic `audit_event_id`, **not implemented/tested** | HIGH | MEDIUM (decided) | implementation + tests of Option A(b), Phase 2.5, and explicit authorization remain before B3 code | yes — implementation/tests + explicit authorization remain B3 preconditions |
| Append→mark_sealed partial-failure idempotency (ADR-023) | (was) open clause unresolved | **Decision made: Option A(b)** — deterministic `audit_event_id` = UUIDv5(org_id+capture_id), as a design constraint. **Implementation not done; tests not done; B3 not authorized.** Still NOT an implementation source. `audit_events.id` is PK so no migration for the minimal guard | HIGH (source-verified) | HIGH (B3) | Implement + test Option A(b) in a future PR (technical plan §8.3/§11); Phase 2.5 remains a separate blocker | yes |
| **Runtime-to-evidence wiring (correction to any "runtime ⇒ evidence captured" assumption)** | Implicit assumption that governed-native runtime produces captured/sealed evidence | `governed-openai.ts:69-70` / `governed-anthropic.ts:71-72` emit via `app.log.info` only; **zero `captureAuditEvent` call-sites in `apps/`**; `/v1/runs` uses `auditAppend` to the chain, not the outbox | HIGH (source-verified) | HIGH (B3 false-confidence risk) | **Decision now exists: ADR-027 (AuditBridge) accepted as a design constraint.** Implementation/tests are absent; direct routes are not yet wired to the outbox; runtime validation/narrowing (`PassthroughInvokedSchema` over `event: unknown`) is not yet implemented; `/v1/runs` stays chain-authoritative via `auditAppend`; B3 remains blocked for product-completeness | yes — implementation/tests (or an accepted deferral naming another authoritative path) remain before B3 |
