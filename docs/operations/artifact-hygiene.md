...
(
  ...
  # Authorized SQL source paths, recomputed from the packaged commit — never a
  # hard-coded list (adding a migration would silently invalidate one). A failure
  # here aborts the block under `set -e`; an empty list rejects every `.sql`.
  TRACKED="$(git -C "$REPO_DIR" ls-tree -r --name-only HEAD)"
  ...
  # dump-shaped name nor a gitleaks-detectable secret, so EVERY extracted `.sql`
  # must BE an authorized source path — membership, not a name heuristic and not
  # directory trust (`…/migrations/backup.sql` is rejected). Compared
  # repo-relative: `./` and §3's `govai-platform/` wrapper are stripped.
  # `grep` exits 1 when nothing is unauthorized, hence `|| true`.
  UNAUTHORIZED_SQL="$( (cd "$CHECK_DIR" && find . -name '*.sql' -print) \
    | sed -e 's|^\./||' -e 's|^govai-platform/||' \
    | LC_ALL=C sort \
    | grep -F -x -v -f "$SQL_ALLOW" || true )"

  if [ -n "$FORBIDDEN" ] || [ -n "$UNAUTHORIZED_SQL" ]; then
    if [ -n "$FORBIDDEN" ]; then printf '%s\n' "$FORBIDDEN"; fi
    if [ -n "$UNAUTHORIZED_SQL" ]; then
      printf '%s\n' "$UNAUTHORIZED_SQL" | sed 's|^|unauthorized .sql (not tracked source): |'
    fi
...
rejecting dump-shaped names outright, and the tracked-source allowlist then
rejects every remaining `.sql` that is not part of the packaged commit — so
`apps/api/src/db/migrations/*.sql` and `infra/postgres/bootstrap.sql` pass
while any other `.sql` does not, whatever its name or directory. `REPO_DIR`
must be the checkout the artifact was packaged from (§2 runs inside it, so the
default `.` is correct; §3 runs from its parent, so use
`REPO_DIR=govai-platform`); if `git` cannot resolve it the block aborts
non-zero rather than accepting the artifact. Membership is by path: the check
proves an extracted `.sql` IS a packaged source path, it does not re-verify
that file's bytes.
